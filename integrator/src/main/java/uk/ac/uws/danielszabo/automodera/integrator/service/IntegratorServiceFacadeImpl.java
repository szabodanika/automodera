...
public class IntegratorServiceFacadeImpl implements IntegratorServiceFacade {
    ...
    public IntegratorServiceFacadeImpl(
            ...
            HashCollectionService collectionService, IntegrationContext integrationContext, ReportService reportService) {
    ...
    public Report checkImage(String image, String attachment, String source) throws IOException {
        ...
        if (highestMatch == null) {
            report = new Report(
                    null, 0, new ArrayList<>(), attachment, source);
        } else {
            ...
            report = new Report(
                    highestMatch, highestMatchScore, highestMatch.getCollection().getTopicList(), attachment, source);
        }
    ...
    @Override
    public List<Collection> requestCollectionRepertoireFromArchive(Node archive) throws TargetNodeUnreachableException {
        ...
        for (Collection collection : collectionRepertoire.getCollectionList()) {
            ...
            if (isSubscribedToAny(collection.getTopicList())) {
                ...
                if (!localHashCollection.isPresent()
                    ...
                    try {
                        collection.setArchive(
                                getKnownNodeById(collection.getArchiveId()).get());
                        if (!localHashCollection.isPresent()) {
                            log.info(
                        ...
                        downloadHashCollection(
                                collection.getArchive().getHost(), collection.getId());
                    } catch (Exception e) {
                        log.error(
