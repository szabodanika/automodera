...
public class IntegratorWebController {
    ...
    // connect to network
    @PostMapping("network")
    public String postNetwork(Model model, @RequestParam String origin,
        ...
        try {
            switch (action) {
                case "test": {
                    NetworkConfiguration networkConfiguration = integratorServiceFacade.fetchNetworkConfiguration(origin);
                ...
                case "connect": {
                    integratorServiceFacade
                            .fetchNetworkConfigurationAndConnect(origin);
                    return "redirect:/";
                }
                case "disconnect": {
    ...
    public String getCollection(
        ...
        if (node != null) {
            ...
            model.addAttribute(
                    "collection", integratorServiceFacade.retrieveHashCollectionById(
                            archive + "/" + collection
                    ).orElse(null));
        }
    ...
    public String getTopic(Model model, @PathVariable String topic) throws Exception {
        ...
        model.addAttribute(
                "collections", integratorServiceFacade.getCollecitonsByTopic(topic));
    ...
    public String postSubscriptionsEdit(Model model, @RequestParam String action, @RequestParam List<String> selected) {
        // redirect to setup page if local node is not set
        if (integratorServiceFacade.getLocalNode() == null) {
            return "redirect:/setup";
        }
    ...
    @PostMapping("test")
    public String postTest(Model model, @RequestParam("image") MultipartFile multipartFile, String attachment)
            throws IOException {
        model.addAttribute("node", integratorServiceFacade.getLocalNode());

        multipartFile.transferTo(Path.of("./temp/" + multipartFile.getOriginalFilename()));
        Report report =
                integratorServiceFacade.checkImage("./temp/" + multipartFile.getOriginalFilename(), attachment, "Web Console");
