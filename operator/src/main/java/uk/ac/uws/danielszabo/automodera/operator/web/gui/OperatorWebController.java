...
public class OperatorWebController {
    ...
    public String getCollection(Model model, @PathVariable("arch") String archive, @PathVariable("coll") String collection) {
        // redirect to setup page if local node is not set
        if (operatorServiceFacade.getLocalNode() == null) {
            return "redirect:/setup";
        }
        ...
        if (node != null) {
            ...
            model.addAttribute(
                    "collection", operatorServiceFacade.retrieveHashCollectionById(
                            archive + "/" + collection
                    ).orElse(null));
        }
