...
public class HashServiceImplTest {
  ...
  public void testPHash() {
    assertDoesNotThrow(
        () -> {
          Hash hash1 = testHashService.pHash(testImageCow);
          // test that the uk.ac.uws.danielszabo.hashnet.operator.service returns a hash
          assertNotNull(hash1);

          Hash hash2 = testHashService.pHash(testImageScooter);
          ...
          Hash hash3 = testHashService.pHash(testImageCow);
    ...
    assertThrows(
        IOException.class,
        () -> {
          Hash hash1 = testHashService.pHash(new File("doesnotexist.jpeg"));
        });
  }
  ...
  public void testSimScore() {
    assertDoesNotThrow(
        () -> {
          ...
          Hash hashCow = testHashService.pHash(testImageCow);
          Hash hashScooter = testHashService.pHash(testImageScooter);
          Hash hashCoin1 = testHashService.pHash(testImageCoin1);
          Hash hashCoin2 = testHashService.pHash(testImageCoin2);
